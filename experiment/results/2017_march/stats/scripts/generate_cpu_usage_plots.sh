...
function parse_stats {
  ...
  cat all_cpu.dat | lua  stats_cdf.lua > $DATA_DIR/$XP/cpu_usage_percentiles_$XP.txt
  cat all_cpu.dat | lua  stats_sum.lua > $DATA_DIR/$XP/cpu_usage_sum_$XP.txt
