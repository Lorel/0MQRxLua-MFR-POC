...
A typical use-case is the deployment of the common filter/map/reduce pattern from the functional programming paradigm~\cite{bird_introduction_1988}, with worker nodes that execute only one function, either \texttt{map}, \texttt{filter} or \texttt{reduce}.
...
Figure~\ref{fig:architecture_pipeline} depicts a possible implementation of the filter/map/reduce pattern using the \SYS middleware.
