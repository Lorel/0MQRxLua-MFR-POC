...
\rp{Does Amazon offers machines with Skylake processors with EPC enabled?}
...
In particular, we use the \zmq's pipeline pattern with the \textsc{Push}-\textsc{Pull} protocol~\cite{zero_mq:pipeline}. \rp{Is 'protocol' the best definition? I would rather call it a 'pattern'}
...
This design allows to dynamically scale up and down each stage of the pipeline in order to adapt it to application's needs or the workload. %It is scalable in that nodes can join at any time.
