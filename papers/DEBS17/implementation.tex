...
Following the observer design pattern~\cite{szallies_using_1997}, hese functions are observers, and the stream is an observable.
...
Our changes to the vanilla Lua source code consist in \vs{FIX} lines of code.
...
A wrapper function is nevertheless installed in the SGX-enabled LuaVM to prevent any of such attempts.
...
The reason is that the assurance that a given code is running inside a SGX-enabled processor is made through the measurement\vs{what kind of measurement is this? size?} of its content when the enclave is created.
%Based on that, further attestation mechanisms guarantee the enclave integrity, which ultimately allows them to receive sensitive information, such as the keys with which sensitive data will be thereafter exchanged.\vs{i don't understand this. to rephrase} 
...
To ease the development of \SYS applications, we statically compiled \emph{json}\vs{ref?} and \emph{csv}\vs{ref?} parsers within our enclaved Lua interpreter.
With these two library, the size of the VM remains reasonably small, approximately \vs{FIX}kb.
