\section{SGX Lightning Tour}\label{sec:background}
...
First, an enclave is created (cf. Figure~\ref{fig:sgx}-\ding{202}).
As soon as a program needs to execute a trusted function (cf. Figure~\ref{fig:sgx}-\ding{203}), it executes SGX's primitive \texttt{ecall} (cf. Figure~\ref{fig:sgx}-\ding{204}).
The call goes through the SGX call gate to bring the execution flow inside the enclave (cf. Figure~\ref{fig:sgx}-\ding{205}).
Once the trusted function is executed by one of the enclave's threads (cf. Figure~\ref{fig:sgx}-\ding{206}), its result is encrypted and sent back (cf. Figure~\ref{fig:sgx}-\ding{207}) before giving back the control to the main processing thread (cf. Figure~\ref{fig:sgx}-\ding{208}).
