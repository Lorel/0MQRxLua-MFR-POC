...
The communication between workers and routers is handled by \zmq, a high-performance asynchronous messaging library~\cite{zero_mq}.
Each router component inbound and outbound queues of fixed size\vs{how much?}.
...
The messages are streamed from a set of anonymous \emph{push} peers (normally the upstream workers in the pipeline).\footnote{\vs{Explain anonymous here.}}
...
\zmq guarantees that the messages are reliabily delivered.
